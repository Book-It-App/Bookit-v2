const HallBooking = require('../model/hallBookingSchema');
...
const createHallBooking = async (req, res, next) => {
  ...
  console.log("create hall booking called");
  try {
    const {
      userId,
      eventManager,
...
const getHallEvents = async (req, res, next) => {
  try {
    const bookings = await HallBooking.find({ isApproved: "Approved By Admin" }).populate('bookedHallId');
...
const getHallBookings = async (req, res, next) => {
  try {
    const bookings = await HallBooking.find().populate('bookedHallId').populate('userId');
...
const getHallBookingById = async (req, res, next) => {
  ...
  try {
    ...
    const booking = await HallBooking.findById(bookingId).populate('bookedHallId').populate('userId');
    // console.log(booking);
    if (!booking) {
...
const getHallBookingByUserId = async (req, res, next) => {
  try {
    // const { userId } = req.params;
    const userId = req.rootUser._id
    const booking = await HallBooking.find({  userId }).populate('bookedHallId').populate('userId');
...
const getHallBookingHod = async (req, res, next) => {
  const hodDepartment = req.rootUser.department
  // console.log(hodDepartment);
  try {
    const bookings = await HallBooking.find({ department: hodDepartment }).populate('bookedHallId');
...
module.exports = { createHallBooking, getHallBookings, getHallBookingById, updateHallBooking, deleteHallBooking, getHallBookingByUserId, getHallEvents,getHallBookingAdmin ,getHallBookingHod};
