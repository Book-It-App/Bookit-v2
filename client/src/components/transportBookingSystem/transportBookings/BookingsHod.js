...
const BookingsHod = () => {
  ...
  const getBookingData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/transport-booking-system/bookingsHod`, {
        withCredentials: true, // include credentials in the request
        headers: {
  ...
  const handleEditClick = (bookingId) => {
    navigate(`/transport-booking-system/bookings/${bookingId}`)
  };
