...
export const CalendarView = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/hall-booking-system/events`,
          {
  ...
  useEffect(() => {
    const fetchHallNames = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/hall-booking-system/halls`
        );
